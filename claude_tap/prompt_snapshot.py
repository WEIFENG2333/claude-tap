...
def infer_provider(record: dict[str, Any]) -> str:
    ...
    if "instructions" in body or "input" in body:
        return "openai"
    if "contents" in body or "system_instruction" in body or "systemInstruction" in body:
        return "gemini"
    return "unknown"
...
def _anthropic_snapshot(record: dict[str, Any]) -> PromptSnapshot:
    ...
    tools = tuple(_anthropic_tools(body.get("tools")))
...
def _prompt_text_for_provider(provider: str, body: dict[str, Any]) -> tuple[str, str, str]:
    if provider == "anthropic":
        return (_anthropic_system_text(body.get("system")), "", _messages_text(body.get("messages"), {"user"}))
    if provider == "openai":
